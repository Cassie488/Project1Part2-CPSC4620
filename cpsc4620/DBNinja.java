...
public final class DBNinja {
	...
	public static ArrayList<Customer> getCustomerList() throws SQLException, IOException
	{
		...
		String discountQuery = "SELECT * FROM customer";
	...
	public static void addToInventory(int toppingID, double quantity) throws SQLException, IOException
	{
		 ...
		 * */
		connect_to_db();
		PreparedStatement InventoryAdd = null;
		String inventory = "UPDATE topping" + "SET topping_CurINVT = topping_CurINVT + ?" + "WHERE topping_TopID = ?";
		InventoryAdd.setDouble(1, quantity);
		InventoryAdd.setInt(2, toppingID);
		InventoryAdd = conn.prepareStatement(inventory);
	...
	public static void printProfitByPizzaReport() throws SQLException, IOException
	{
		...
		while (rsView.next()) {
			...
			System.out.printf("%-10s\t\t%-11s\t\t%-6.2f\t\t%-15s\n",size, crust, profit, month);
		}
		conn.close();
	}
	...
	public static void printProfitByOrderType() throws SQLException, IOException
	{
		...
		while (rsView.next()) {
			...
			Float profit = rsView.getFloat("Profit");
			System.out.printf("%-13s\t\t%-11s\t\t%-17.2f\t\t%-16.2f\t\t%-6.2f\n",(type == null) ? " " : type,month,price,cost,profit);
		}
		conn.close();
	}
